...
import {
	html, css, SpectreDialog, askForPassword, SPR,
	formatForMachine, getLocalWallet
} from './spectre-dialog.js';
const pass = "";
import {Wallet} from '@spectre/wallet-worker';

class SpectreSendDialog extends SpectreDialog{
	...
	static get styles(){
		return [SpectreDialog.styles, 
		css`
			.container{
				max-height:var(--spectre-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'spectre' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in SPR')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${SPR(dataFee)} SPR</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${SPR(fee)} SPR</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${SPR(totalAmount)} SPR</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] SPR) is too large.');
			msg = msg.replace('[n]', SPR(estimate.fee));
...
SpectreSendDialog.define("spectre-send-dialog");
