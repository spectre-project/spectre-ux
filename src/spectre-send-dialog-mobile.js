...
class SpectreSendDialogMobile extends SpectreDialog{
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${SPR(totalAmount)} SPR</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in SPR", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] SPR) is very large.');
...
SpectreSendDialogMobile.define("spectre-send-dialog-mobile");
