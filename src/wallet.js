...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@spectre/wallet-worker';
export const {Deferred, SPR, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.SpectreConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("SpectreUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initSpectreFramework, Storage} = require("spectre-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("spectre-grpc-node");
*/
