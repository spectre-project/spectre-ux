import {
	html, css, FlowFormat, SpectreWalletUI, dpc,
	baseUrl, SPR, renderPagination, buildPagination, paginationStyle,
...
} from './spectre-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class SpectreWalletMobile extends SpectreWalletUI{
	...
	static get styles(){
		return [SpectreWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--spectre-wallet-header-padding, 5px 10px);
				margin:var(--spectre-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--spectre-pagination-box-padding, 10px 5px;);
			}

			.logo{
				width:30px;height:30px;/*background-color:#DDD*/
				display:var(--spectre-wallet-header-logo-diaplay, initial);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--spectre-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--spectre-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.tab.selected{
				border-bottom-color:var(--spectre-wallet-tab-active-border-color, var(--flow-primary-color));
			}
			...
			.address-input{
				...
				min-width:var(--spectre-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--spectre-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--spectre-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--spectre-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--spectre-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--spectre-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--spectre-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--spectre-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
	...
	render(){
		...
		let donationAddresses = [
			["Spectre Devfund donations:", "spectre:qzrq7v5jhsc5znvtfdg6vxg7dz5x8dqe4wrh90jkdnwehp6vr8uj7csdss2l7"],
			["Spectre WebWallet donations:", "spectre:qqe3p64wpjf5y27kxppxrgks298ge6lhu6ws7ndx4tswzj7c84qkjlrspcuxw"],
		]
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">SPECTRE WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the spectre ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Spectre Core SPR:</span> ${window.PWA_MODULES['@spectre/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Spectre Wallet Framework SPR:</span> ${window.PWA_MODULES['@spectre/wallet']}</div>
							<div class="badge"><span is="i18n-span">Spectre gRPC:</span> ${window.PWA_MODULES['@spectre/grpc']}</div>
							<div class="badge"><span is="i18n-span">Spectre gRPC Relay:</span> ${window.PWA_MODULES['@spectre/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Spectre UX SPR:</span> ${window.PWA_MODULES['@spectre/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX SPR:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">SPECTRE FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] SPR</b> available.', SPR(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${SPR(inUseUTXOs.satoshis||0)} SPR</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="${baseUrl+'/resources/images/spectre.png'}"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.spectre-network.org/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatSPR(total)+' SPR':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatSPR(available)} SPR</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatSPR(pending)} SPR</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'spectretest:abc'
