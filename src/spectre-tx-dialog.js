import {
	html, css, SpectreDialog, SPR, txListStyle,
	paginationStyle, buildPagination,
	renderPagination, T
} from './spectre-dialog.js';

class SpectreTXDialog extends SpectreDialog{
	...
	static get styles(){
		return [SpectreDialog.styles, paginationStyle, txListStyle,
		css`
			:host{
				position:fixed;
			...
			.container{
				max-height:var(--spectre-dialog-container-max-height, 90%);
				max-width:var(--spectre-dialog-container-max-width, 90%)
			}
...
SpectreTXDialog.define("spectre-tx-dialog");
