...
export class SpectreDialog extends BaseElement{
	...
	static get styles(){
		return [ScrollbarStyle, SpinnerStyle, css`
			:host{
				...
				position:var(--spectre-dialog-position, absolute);
				top:0px;
				left:0px;
				width:100%;
				height:100%;
				background-color:var(--spectre-dialog-bg-color);
			...
			.container{
				box-sizing:border-box;
				width:100%;
				height:var(--spectre-dialog-container-height, calc(100% - 10px));
				background-color:var(--flow-background-color, #F00);
				z-index:1;
				border:var(--spectre-dialog-container-border, 2px solid var(--flow-primary-color));
				border-radius:var(--spectre-dialog-container-border-radius, 3px);
				max-width:var(--spectre-dialog-container-max-width, 700px);
				max-height:var(--spectre-dialog-container-max-height, 300px);
				margin:var(--spectre-dialog-container-margin, 5px auto);
				padding:var(--spectre-dialog-container-padding, 0px);
				position:relative;
			...
			:host([hideable]) .close-btn{
				display:var(--spectre-dialog-container-close-btn-display, inline-block)
			}
			.heading{
			...
			[hidden]{display:none}
			.buttons{margin:var(--spectre-dialog-buttons-margin, 10px auto);display:flex;width:var(--spectre-dialog-buttons-width,90%);}
	...
	pushHistory(uid=""){
		...
		name = name.replace(/\-/g, "");
		let key = name.toLowerCase().replace(/(spectre|dialog|mobile)/g, '')
