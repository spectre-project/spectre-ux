...
import {
	Deferred, GetTS, SPR, formatForMachine, formatForHuman,
...
import {initSpectreFramework, Wallet, workerLog} from '@spectre/wallet-worker';
export {Wallet};
Wallet.setWorkerLogLevel(localStorage.walletWorkerLogLevel || 'none')

export {html, css, FlowFormat, dpc, baseUrl, debug};

export class SpectreWalletUI extends BaseElement{
	...
	async initNetworkSettings() {
		...
		//const { network, port } = this.local_spectred_settings;
			...
			<div class="tx-notifications">
				${notifications.map(n=>{
					return html`<div class="tx-notification">
						${n.compoundUTXOs?
							T(`Compounding UTXOs...`):
							i18n.t(`Preparing transaction for [n] SPR ....`)
							.replace('[n]', this.formatSPR(n.amount))}
					</div>`
				})}
				...
				return html`
					<flow-expandable class="tx-row" static-icon expand ?txin=${tx.in} ?txmoved=${tx.isMoved} ?txout=${!tx.in}
						...
						<div class="tx-title" slot="title">
							...
							<div class="amount">
								${tx.in?'':'-'}${this.formatSPR(tx.amount)} SPR
							</div>
						</div>
						...
						<div class="tx-body">
							${tx.note}
							<div class="tx-id">
								<a target="_blank" href="https://explorer.spectre-network.org/txs/${tx.id}">${tx.id}</a>
							</div>
							<div class="tx-address">
								${tx.myAddress?T('COMPOUNDING WALLET => '):''}
								<a target="_blank" href="https://explorer.spectre-network.org/addresses/${tx.address}">${tx.address}</a>
							</div>
						</div>
					</flow-expandable>
				`
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?txin=${tx.in} tx-version=${tx.version} ?txmoved=${tx.isMoved} ?txout=${!tx.in}>
						...
						<div class="tx-amount">${tx.in?'':'-'}${SPR(tx.amount)} SPR</div>
						<div class="br tx-note">${tx.note}</div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.spectre-network.org/txs/${tx.id.split(":")[0]}">${tx.id.split(":")[0]}</a>
						</div>
						<div class="tx-address">
							${tx.myAddress?T('COMPOUNDING WALLET => '):''}
							<a target="_blank" href="https://explorer.spectre-network.org/addresses/${tx.address}">${tx.address}</a>
		...
		items.map((tx, i)=>{
			...
			fields.forEach(field=>{
				switch(field){
					...
					break;
					case 'amount':
						row.push(escape(`${tx.in?'':'-'}${SPR(tx.amount)}`))
					break;
					case 'direction':
			...
			<div class="tx-list">
				${items.map((tx, i)=>{
					...
					<div class="tx-row" ?iscoinbase=${!tx.isCoinbase}>
						...
						<div class="tx-amount">${SPR(tx.satoshis)} SPR</div>
						<div class="br tx-mass"></div>
						<div class="br tx-id">
							<a target="_blank" href="https://explorer.spectre-network.org/txs/${tx.txId}">${tx.id}</a>
						</div>
						<div class="tx-address">
							<a target="_blank" href="https://explorer.spectre-network.org/addresses/${tx.address}">${tx.address}</a>
						</div>
	...
	async alertUTXOIndexSupportIssue(){
		...
		let body = html`${i18nHTMLFormat(`'utxoindex' flag is missing from SPECTRED config.<br />
	...
	connectedCallback(){
		...
		let openDialog = document.createElement('spectre-open-dialog');
		openDialog.hideLogo = !!this.hideOpenWalletLogo;
		this.parentNode.insertBefore(openDialog, this.nextSibling)
		this.sendDialog = document.createElement("spectre-send-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.sendDialog);
		this.receiveDialog = document.createElement("spectre-receive-dialog"+mobileSuffix);
		this.parentNode.appendChild(this.receiveDialog);
		this.seedsDialog = document.createElement("spectre-seeds-dialog");
		this.parentNode.appendChild(this.seedsDialog);
		let t9Dialog = document.createElement("spectre-t9-dialog");
		this.parentNode.appendChild(t9Dialog);
		let qrscannerDialog = document.createElement("spectre-qrscanner-dialog");
		qrscannerDialog.debug = this.debugscanner
		this.parentNode.appendChild(qrscannerDialog);
		let uploadFileDialog = document.createElement("spectre-upload-file-dialog");
		this.parentNode.appendChild(uploadFileDialog);

		const {workerCorePath} = window.SpectreConfig||{}
		initSpectreFramework({
			workerPath: workerCorePath||"/spectre-wallet-worker/worker.js?ident="+(window.SpectreConfig?.ident||"")
		}).then(()=>{
			let encryptedMnemonic = getLocalWallet()?.mnemonic;
	...
	async handleInitDialogCallback({dialog, password, seedPhrase, encryptedMnemonic}){
		...
		if(!rpc)
			return FlowDialog.alert(i18n.t("Error"), i18n.t("Spectre Daemon config is missing."));
	...
	showTxDialog(){
		if(!this.txDialog){
			this.txDialog = document.createElement("spectre-tx-dialog");
	...
	async isValidAddress(address){
		...
		if(window.mobileMode && prefix=="spectretest")
			return true;
	...
	*/
	...
	async getSpectreFromFaucet(amount) {
	...
	requestFaucetFunds() {
		...
		showT9({
			...
			inputLabel:i18n.t('Amount in SPR')
		}, ({value:amount, dialog})=>{
			let sompis = formatForMachine(amount||0);
			if(sompis > this.faucetFundsAvailable){
				let msg = i18n.t(`You can't request more than [n] SPR.`)
						.replace("[n]", SPR(this.faucetFundsAvailable||0))
				return dialog.setError(msg);//'
			}
			
			dialog.hide();

			this.getSpectreFromFaucet(amount)
