import {html, css, SpectreDialog, i18n, T} from './spectre-dialog.js';
import {askForPassword, baseUrl} from "./wallet.js";
const pass = "";

class SpectreOpenDialog extends SpectreDialog{
	...
	static get styles(){
		return [SpectreDialog.styles, css`
			.container{max-height:var(--spectre-dialog-container-max-height, 600px)}
			:host([mode="create"]) .container{max-height:var(--spectre-dialog-container-max-height, 500px)}
			:host([mode="init"]) .container{max-height:var(--spectre-dialog-container-max-height, 200px)}
			:host([mode="recover"]) .container{max-height:var(--spectre-dialog-container-max-height, 450px)}
			.buttons{justify-content:center;--spectre-dialog-buttons-width:100%;}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to Spectre Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="${baseUrl+'/resources/images/spectre.png'}" />
			</div>`}
...
SpectreOpenDialog.define("spectre-open-dialog");
